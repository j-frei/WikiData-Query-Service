...
while [ ! curl -Is http://wikidata-query-service:9999/bigdata 2>/dev/null | grep "HTTP/1.1" 1>/dev/null ]
do
...
if [ ! curl -Is http://wikidata-query-service:9999/bigdata 2>/dev/null | grep "HTTP/1.1" 1>/dev/null ]; then
